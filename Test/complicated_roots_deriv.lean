...
theorem real_roots_deriv (n : ℕ) (x : ℕ → ℝ) : ∃ (y : ℕ → ℝ), derivative (∏ k in range n, (X - C (x k))) = (C (n:ℝ)) * (∏ k in range (n-1), (X - C (y k))) := by
  ...
  have eachRootReal : ∀ (k : ℕ), (k ∈ range m) → (z k).im = 0 := by
    intro k hk
    have hroot : (z k).im * (eval (z k) P'_C) = 0 := by
      rw [hz]
      simp [hk]
      right
      right
      ...
      simp
    rw [<- eval₂_eq_eval_map] at hroot
    have eq1 : P' = derivative (∏ k in range (m + 1), (X - C (x k))) := by rfl
    rw [prod_eq_multiset_prod, derivative_prod] at eq1
    have eq2 : P' = Multiset.sum (Multiset.map (fun (i:ℕ) => Multiset.prod (Multiset.map (fun k => X - C (x k)) (Multiset.erase (range (m + 1)).val i))) (range (m + 1)).val) := by
      rw [eq1]
      congr
      funext i
      suffices : derivative (X - C (x i)) = 1
      . rw [this]
        ring
        congr!
      apply [derivative_X_sub_C]
    rw [eq2, eval₂_multiset_sum, Multiset.map_map] at hroot
    clear eq1 eq2  
...
--    change eval (z k) (Polynomial.map Complex.ofReal (derivative P)) = 0 at hroot
--    rw [eq1, <-coe_mapRingHom, <-Multiset.sum_hom, Multiset.map_map] at hroot
--    clear eq1
